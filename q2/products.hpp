...
public:
	...
	Future(string _productId, FutureType _futureType, string _underlyingAssetTicker, date _maturityDate) : Product(_productId, FUTURE) {
		productId = _productId;
	...
	friend ostream& operator<<(ostream& output, const Future& future) {
		output << future.futureType << " " << future.underlyingAssetTicker << " " << future.maturityDate;
		return output;
	}
private:
	string productId; // product identifier variable
